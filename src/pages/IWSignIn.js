...
const IWSignin = () => {
  ...
  const handleSignInClick = async (e) => {
    ...
    try {
      ...
      const q = query(usersRef, where("code_interimaire", "==", codeInterimaire));
      ...
      if (userData.passwordHash === password) {
        ...
        if (userData.role === "interim_collaborator") {
          console.log("Interim worker signed in successfully:", codeInterimaire);
          navigate("/IWDashboard"); // Redirect to the IW dashboard upon successful sign-in
        } else {
          setErrorMessage("You are not authorized to sign in as an interim worker.");
